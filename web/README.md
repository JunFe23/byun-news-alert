...
(async () => {
  for (const [day, next] of [['2026-05-21','2026-05-22'],['2026-05-22','2026-05-23']]) {
    ...
    const { data: deals } = await sb.from('fa_players').select('player_name,contract_status').gte('status_updated_at', since).lt('status_updated_at', until).in('contract_status', ['잔류','이적','계약미체결','미정']);
